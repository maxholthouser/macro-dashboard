...
def fetch_all_series():
    series_map = {}
    failures = []
    fred_timeout_streak = 0
    fred_circuit_open = False
    ...
    for index, (key, config) in enumerate(SERIES_CONFIG.items(), start=1):
        ...
        try:
            ...
            if "curl exit 28" in str(error):
                fred_timeout_streak += 1
                if fred_timeout_streak >= FRED_TIMEOUT_STREAK_LIMIT:
                    fred_circuit_open = True
                    failures.append("FRED circuit breaker opened after repeated timeouts; remaining FRED series were skipped for this refresh.")
                    log("Opening FRED circuit breaker")
            else:
                fred_timeout_streak = 0
    return series_map, failures, fred_circuit_open
...
def build_sections(series_map):
    return [
        {"kicker": "Growth", "title": "Growth & Activity", "metrics": [
        ...
        {"kicker": "Labor", "title": "Labor Market", "metrics": [
        ...
        {"kicker": "Inflation", "title": "Inflation", "metrics": [
        ...
        {"kicker": "Policy", "title": "Monetary Policy & Rates", "metrics": [
        ...
        {"kicker": "Markets", "title": "Financial Conditions", "metrics": [
        ...
        {"kicker": "Trade", "title": "External, Leading & Fiscal", "metrics": [
...
def build_payload():
    ...
    series_map, failures, fred_circuit_open = fetch_all_series()
    ...
    if fred_circuit_open:
        notes.append("FRED was timing out from the refresh runner, so remaining FRED series were skipped after repeated failures to keep the dashboard current.")
